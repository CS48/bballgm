...
export class StatsCalculator {
  ...
  public calculateTeamStats(playerStats: PlayerSeasonStats[]): TeamSeasonStats {
    ...
    const totals = playerStats.reduce((acc, player) => ({
      ...
      turnovers: 0, fg_made: 0, fg_attempted: 0, three_made: 0, three_attempted: 0,
   ...
   */
  public updateSeasonStats(currentStats: PlayerSeasonStats, gameStats: {
    minutes: number;
    points: number;
    rebounds: number;
    assists: number;
    ...
    updatedStats.fg_pct = updatedStats.fg_attempted > 0 ? this.roundToDecimal(updatedStats.fg_made / updatedStats.fg_attempted, 3) : 0;
    updatedStats.three_pct = updatedStats.three_attempted > 0 ? this.roundToDecimal(updatedStats.three_made / updatedStats.three_attempted, 3) : 0;
    updatedStats.ft_pct = updatedStats.ft_attempted > 0 ? this.roundToDecimal(updatedStats.ft_made / updatedStats.ft_attempted, 3) : 0;
  ...
  public calculateLeagueStats(allTeamStats: TeamSeasonStats[]): {
    ...
    if (allTeamStats.length === 0) {
      return {
        league_ppg: 0, league_fg_pct: 0, league_three_pct: 0, league_ft_pct: 0,
        league_rpg: 0, league_apg: 0, league_spg: 0, league_bpg: 0, league_tpg: 0
      };
    }

    const totals = allTeamStats.reduce((acc, team) => ({
      ...
      ppg: 0, fg_pct: 0, three_pct: 0, ft_pct: 0, rpg: 0, apg: 0, spg: 0, bpg: 0, tpg: 0
    });
  ...
  public calculatePER(playerStats: PlayerSeasonStats): number {
    ...
    const per = (playerStats.points + playerStats.rebounds + playerStats.assists + 
                 playerStats.steals + playerStats.blocks - playerStats.turnovers) / playerStats.games;
  ...
  public calculateUsageRate(playerStats: PlayerSeasonStats, teamStats: TeamSeasonStats): number {
    ...
    const usageRate = (playerStats.fg_attempted + 0.44 * playerStats.ft_attempted + playerStats.turnovers) / 
  ...
  public calculateWinShares(playerStats: PlayerSeasonStats, teamWins: number, teamGames: number): number {
    ...
    const playerContribution = (playerStats.points + playerStats.rebounds + playerStats.assists) / 
                              (playerStats.games * 20); // Normalize to per-game contribution
  ...
  private getEmptyCareerStats(): PlayerCareerStats {
    return {
      seasons: 0, games: 0, minutes: 0, points: 0, ppg: 0, rebounds: 0, rpg: 0,
      assists: 0, apg: 0, steals: 0, spg: 0, blocks: 0, bpg: 0, turnovers: 0, tpg: 0,
  ...
  private getEmptyTeamStats(): TeamSeasonStats {
    return {
      games: 0, wins: 0, losses: 0, ppg: 0, opp_ppg: 0, fg_pct: 0, three_pct: 0,
      ft_pct: 0, rpg: 0, apg: 0, spg: 0, bpg: 0, tpg: 0
    };
  }
  ...
  public calculateAdvancedStats(playerStats: PlayerSeasonStats): {
    ...
    const efgPct = playerStats.fg_attempted > 0 ? 
      (playerStats.fg_made + 0.5 * playerStats.three_made) / playerStats.fg_attempted : 0;
