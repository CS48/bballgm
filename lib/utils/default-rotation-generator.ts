...
export function createDefaultRotationConfig(
  ...
  starters.forEach((starter, index) => {
    if (index < 3) {
      ...
      playerRotations.push({
        ...
        active_minutes: [[0, 6], [12, 18], [24, 30], [36, 42], [42, 48]],
        total_minutes: 32
      })
    } else {
      ...
      playerRotations.push({
        player_id: starter.player_id,
        active_minutes: [[0, 6], [12, 18], [24, 30], [36, 42], [42, 46]],
        total_minutes: 30
      })
    }
  })
  ...
  if (backups.length === 5) {
    ...
    backups.forEach(backup => {
      ...
      if (plays46_48) {
        ...
        playerRotations.push({
          ...
          active_minutes: [[6, 12], [18, 24], [30, 36], [46, 48]],
          total_minutes: 22
        })
      } else {
        ...
        playerRotations.push({
          player_id: backup.player_id,
          active_minutes: [[6, 12], [18, 24], [30, 36]],
          total_minutes: 18
        })
      }
    })
    ...
    missingPositions.forEach(pos => {
      const fillPlayer = thirdString.find(p => p.position === pos) || 
                        playersByPosition[pos][1] || 
