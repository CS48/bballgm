...
export class RotationManager {
  ...
  private getDefaultActiveLineup(): SimulationPlayer[] {
    ...
    const starters: SimulationPlayer[] = positions.map((pos) => playersByPosition[pos][0]).filter(Boolean);

    // Get backups: 2nd best at each position
    const backups: SimulationPlayer[] = positions.map((pos) => playersByPosition[pos][1]).filter(Boolean);

    // Get third string: 3rd best at each position
    const thirdString: SimulationPlayer[] = positions.map((pos) => playersByPosition[pos][2]).filter(Boolean);
  ...
  private getActiveLineupFromConfig(): SimulationPlayer[] {
    ...
    for (const playerRotation of this.rotationConfig.player_rotations) {
      ...
      const isActive = playerRotation.active_minutes.some(
        ([start, end]) => totalGameTime >= start * 60 && totalGameTime < end * 60
      );
  ...
  private handleFoulTrouble(lineup: SimulationPlayer[]): SimulationPlayer[] {
    ...
    return lineup.map((player) => {
      ...
      const shouldSub = (quarter <= 2 && fouls >= 2) || (quarter === 3 && fouls >= 4) || fouls >= 5;
  ...
  private findBestSubstitute(position: string, currentLineup: SimulationPlayer[]): SimulationPlayer | null {
    const availablePlayers = this.team.players.filter((p) => !currentLineup.includes(p) && p.position === position);
  ...
  private getBestLineup(availablePlayers: SimulationPlayer[], count: number): SimulationPlayer[] {
    ...
    const sortedPlayers = [...availablePlayers].sort(
      (a, b) => this.calculateOverallRating(b) - this.calculateOverallRating(a)
    );
    ...
    for (const position of positions) {
      ...
      const playerAtPosition = sortedPlayers.find((p) => p.position === position && !lineup.includes(p));
