...
console.log('\nAge Distribution:');
console.log(
  `  Young (19-22):     ${youngPlayers} (${((youngPlayers / ages.length) * 100).toFixed(1)}%) - Target: ~23%`
);
console.log(
  `  Prime (23-27):     ${primePlayers} (${((primePlayers / ages.length) * 100).toFixed(1)}%) - Target: ~57%`
);
console.log(
  `  Veterans (28-32):  ${veteranPlayers} (${((veteranPlayers / ages.length) * 100).toFixed(1)}%) - Target: ~17%`
);
console.log(
  `  Late Career (33+): ${lateCareerPlayers} (${((lateCareerPlayers / ages.length) * 100).toFixed(1)}%) - Target: ~3%`
);
...
tiers.forEach((tier) => {
  ...
  const percentage = ((count / overallRatings.length) * 100).toFixed(1);
  console.log(
    `  ${tier.name.padEnd(24)} ${count.toString().padStart(3)} (${percentage.padStart(5)}%) - Target: ${tier.target}`
  );
});
