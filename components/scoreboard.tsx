...
export function Scoreboard({ awayTeam, homeTeam, gameState }: ScoreboardProps) {
  ...
  const getGameState = () => {
    ...
    const isHalftime =
      (gameState.currentQuarter === 3 && gameState.quarterTimeRemaining === 12 * 60) ||
      (lastEvent?.description?.includes('End of Quarter 2') &&
        gameState.currentQuarter === 3 &&
        gameState.quarterTimeRemaining === 12 * 60);
          ...
          {gameStateInfo.label === 'Halftime' || gameStateInfo.label === 'Game Over' ? (
            <div className="w-24 h-24 rounded-full bg-gray-100 flex items-center justify-center border-2 border-gray-300">
              <div className="text-center">
                <div className="text-xs font-medium text-gray-600 uppercase">{gameStateInfo.label}</div>
              </div>
            </div>
          ) : (
            <>
              <div className="text-sm font-medium text-gray-600 uppercase">{gameStateInfo.label}</div>
